/**
 * Super Admin Calendar Setup Page
 * Allows Super Admins to connect and manage their calendar systems
 */
...
export default function AdminCalendarSetup() {
  ...
  // Fetch calendar connections (using admin endpoint)
  const { data: connectionsData, isLoading } = useQuery<{ connections: CalendarConnection[] }>({
    queryKey: ['/api/admin/calendar/setup/connections'],
    queryFn: async () => {
      const response = await fetch('/api/admin/calendar/setup/connections');
  ...
  const addConnectionMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.requiresOAuth && data.authUrl) {
        ...
        queryClient.invalidateQueries({ queryKey: ['/api/admin/calendar/setup/connections'] });
  ...
  const deleteConnectionMutation = useMutation({
    mutationFn: async (connectionId: string) => {
      const response = await fetch(`/api/admin/calendar/setup/connections/${connectionId}`, {
        method: 'DELETE',
      });
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ id, isActive }: { id: string; isActive: boolean }) => {
      const response = await fetch(`/api/admin/calendar/setup/connections/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const testConnectionMutation = useMutation({
    mutationFn: async (connectionId: string) => {
      const response = await fetch(`/api/admin/calendar/setup/connections/${connectionId}/test`, {
        method: 'POST',
      });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto py-8 px-4">
        <PageHeader
          title="Super Admin Calendar Setup"
          description="Connect and manage your operational calendars for Super Admin unified calendar"
        ...
        <Card className="mb-6 border-blue-200 bg-blue-50">
          <CardContent className="pt-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-medium text-blue-900 mb-1">Super Admin Calendar</h4>
                <p className="text-sm text-blue-800">
                  This is your personal Super Admin calendar setup. Connect your calendars to sync with the unified calendar view.
                  This is separate from Enterprise user calendars.
                </p>
              </div>
            </div>
          </CardContent>
