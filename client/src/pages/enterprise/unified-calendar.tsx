/**
 * Enterprise Unified Calendar View
 * Displays events from all connected calendars for Enterprise users (isolated per user)
 */
...
export default function EnterpriseUnifiedCalendar() {
  ...
  const { data: calendarData, isLoading, refetch } = useQuery<{
    ...
    queryKey: ['/api/enterprise/calendar/unified', dateRange, sourceFilter],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/enterprise/calendar/unified?${params.toString()}`);
  ...
  const syncMutation = useMutation({
    mutationFn: async (source?: string) => {
      const response = await fetch('/api/enterprise/calendar/sync', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto py-8 px-4">
        <PageHeader
          ...
          description="Your operational calendar - view all events from your connected calendars"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <StatCard
            title="Connected"
            value={Object.keys(eventsBySource).length}
            icon={<CheckCircle />}
          />
          <StatCard
            title="Sources"
            value={Object.values(eventsBySource).reduce((a, b) => a + b, 0) > 0 ? Object.keys(eventsBySource).length : 0}
            icon={<ExternalLink />}
          />
        </div>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <Button onClick={() => setLocation('/enterprise/calendar/setup')} variant="outline">
          ...
          <TabsContent value="calendar" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Showing {calendarData?.events.length || 0} events from all connected calendars
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Connect your calendars to start syncing events.
                    </p>
                    <Button onClick={() => setLocation('/enterprise/calendar/setup')}>
                      <Settings className="w-4 h-4 mr-2" />
                      Set Up Calendars
                    </Button>
                  </div>
