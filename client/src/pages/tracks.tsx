...
interface Track {
  ...
  isPublished: boolean;
  createdAt: string;
  lessonCount?: number; // Number of lessons for this track
  aiTutor: {
    id: string;
    name: string;
    specialty: string;
...
export default function Tracks() {
  ...
  const tracks = apiTracks && apiTracks.length > 0 ? 
    apiTracks.map(track => ({
      ...
      duration: "8 weeks",
      lessons: track.lessonCount || 0, // Use actual lesson count from API
      hasLessons: (track.lessonCount || 0) > 0, // Check if track has lessons
      ...
      ...track,
      hasLessons: true, // Mock tracks assume they have lessons
      aiTutor: undefined
    }));
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
        ...
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-2 leading-tight">Learning Tracks</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
          <Card>
            <CardContent className="p-3 sm:p-4 md:p-6">
              <div className="flex items-center">
                <BookOpen className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-blue-600 mr-2 sm:mr-3 flex-shrink-0" />
                <div className="min-w-0">
                  <p className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight">{mockTracks.length}</p>
                  <p className="text-xs sm:text-sm text-slate-600 leading-tight">Total Tracks</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-3 sm:p-4 md:p-6">
              <div className="flex items-center">
                <Users className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-green-600 mr-2 sm:mr-3 flex-shrink-0" />
                <div className="min-w-0">
                  ...
                  <p className="text-xs sm:text-sm text-slate-600 leading-tight">Active Students</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-3 sm:p-4 md:p-6">
              <div className="flex items-center">
                <Award className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-purple-600 mr-2 sm:mr-3 flex-shrink-0" />
                <div className="min-w-0">
                  <p className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight">
                    {mockTracks.reduce((sum, track) => sum + track.lessons, 0)}
                  </p>
                  <p className="text-xs sm:text-sm text-slate-600 leading-tight">Total Lessons</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-3 sm:p-4 md:p-6">
              <div className="flex items-center">
                <CheckCircle className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-orange-600 mr-2 sm:mr-3 flex-shrink-0" />
                <div className="min-w-0">
                  <p className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight">0%</p>
                  <p className="text-xs sm:text-sm text-slate-600 leading-tight">Your Progress</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
                <div className="animate-pulse">
                  ...
                  <div className="h-10 sm:h-8 bg-gray-200 rounded w-full"></div>
                </div>
              </div>
            ))}
          </div>
            ...
            <Card key={track.id} className="hover:shadow-lg transition-shadow duration-200">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  {/* Action Button - Only show if track has lessons */}
                  {track.hasLessons ? (
                    <Button 
                    ...
                    <Button 
                      ...
                      data-testid={`button-no-lessons-${track.id}`}
                    >
                      Lessons Coming Soon
                    </Button>
                  )}
        ...
        <div className="mt-8 sm:mt-12 text-center">
          <Card className="bg-gradient-to-r from-blue-50 to-slate-50 border-blue-200">
            <CardContent className="p-4 sm:p-6 md:p-8">
              <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-3 sm:mb-4 leading-tight">
                Ready to Advance Your Diving Career?
              </h3>
              <p className="text-sm sm:text-base md:text-lg text-slate-600 mb-4 sm:mb-6 max-w-2xl mx-auto leading-relaxed">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                <Link href="/learning-path" className="w-full sm:w-auto">
                  <Button size="lg" className="bg-blue-600 hover:bg-blue-700 w-full sm:w-auto min-h-[48px] text-sm sm:text-base">
                    Get AI Recommendations
                  </Button>
                </Link>
                <Link href="/affiliate" className="w-full sm:w-auto">
                  <Button size="lg" variant="outline" className="w-full sm:w-auto min-h-[48px] text-sm sm:text-base">
