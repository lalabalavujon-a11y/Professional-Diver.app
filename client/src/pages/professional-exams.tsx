...
import { Clock, FileText, Mic, Brain, BarChart3, Award, CheckCircle, Play, Timer, Volume2, Target } from "lucide-react";
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-3 pt-2">
                  {exam.attempts < exam.maxAttempts && (
                    <div className="space-y-2">
                      <div className="text-xs font-medium text-slate-600 mb-1">Choose Exam Mode:</div>
                      <div className="grid grid-cols-2 gap-2">
                        <Link href={`/exams/${exam.slug}/start?mode=srs`} className="flex-1">
                          <Button 
                            variant="outline"
                            className="w-full border-blue-200 text-blue-600 hover:bg-blue-50 text-sm"
                            data-testid={`button-srs-mode-${exam.slug}`}
                          >
                            <Brain className="w-3 h-3 mr-1" />
                            SRS Learning
                          </Button>
                        </Link>
                        <Link href={`/exams/${exam.slug}/start?mode=full-exam`} className="flex-1">
                          <Button 
                      ...
                      <div className="text-xs text-slate-500 space-y-1">
                        <div className="flex items-center">
                          ...
                          <span>SRS: {Math.min(15, exam.questionsCount)} questions for focused learning</span>
                        </div>
                        <div className="flex items-center">
                          <Target className="w-3 h-3 mr-1 text-orange-500" />
                          <span>Full: {exam.questionsCount} questions, {exam.timeLimit} min (real exam simulation)</span>
                        </div>
                      </div>
                    </div>
                  )}
