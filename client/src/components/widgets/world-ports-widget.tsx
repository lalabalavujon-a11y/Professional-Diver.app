...
import { Anchor, Search, Phone, Radio, MapPin, AlertCircle, Loader2 } from 'lucide-react';
...
// Fallback port data in case API fails
const fallbackPortsData: Port[] = [
...
export default function WorldPortsWidget() {
  const [searchTerm, setSearchTerm] = useState('');
  const [regionFilter, setRegionFilter] = useState<string>('all');
  const [typeFilter, setTypeFilter] = useState<string>('all');

  // Fetch ports from API
  const { data: portsResponse, isLoading, isError } = useQuery<{ ports: Port[] }>({
    queryKey: ['/api/ports', regionFilter, typeFilter, searchTerm],
    queryFn: async () => {
  ...
  const portsData = portsResponse?.ports || fallbackPortsData;
  const showError = isError && !portsResponse;

  const regions = useMemo(() => {
    const uniqueRegions = Array.from(new Set(portsData.map(port => port.region))).sort();
    return uniqueRegions;
  }, [portsData]);

  const filteredPorts = useMemo(() => {
    // If API is working, ports are already filtered server-side
    // But we can add client-side search filtering as well for better UX
    if (searchTerm) {
      return portsData.filter(port => {
        return (
          port.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          port.country.toLowerCase().includes(searchTerm.toLowerCase()) ||
          port.callsign.toLowerCase().includes(searchTerm.toLowerCase())
        );
      });
    }
    return portsData;
  }, [portsData, searchTerm]);
    ...
    <Card className="w-full">
      ...
      <CardContent className="p-0">
        <div className="p-4 space-y-3 border-b">
          ...
          <div className="space-y-2">
            <div className="relative">
              ...
              <Input
                placeholder="Search ports, country, or callsign..."
                value={searchTerm}
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="font-semibold text-sm text-slate-900">{port.name}</h4>
                        <Badge variant={port.type === 'Primary' ? 'default' : 'secondary'} className="text-xs">
                          {port.type}
                        </Badge>
                      </div>
                  ...
                  <div className="grid grid-cols-1 gap-2 text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono">{port.callsign}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Radio className="w-3 h-3 text-blue-600" />
                      <span className="font-medium">VHF:</span>
                      <span>{port.vhfChannels.join(', ')}</span>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    {port.email && (
                      <div className="flex items-center gap-2 text-xs text-blue-600">
                        ...
                        <a href={`mailto:${port.email}`} className="hover:underline">{port.email}</a>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="px-4 py-2 border-t bg-slate-50 text-xs text-slate-600">
          Showing {filteredPorts.length} of {portsData.length} ports
        </div>
      </CardContent>
    </Card>
  );
