...
// Proper lesson content for Communication Underwater
const communicationLessonContent = `# Communication Underwater
...
async function fixAllAlignment() {
  console.log('🔧 Fixing all alignment issues...\n');

  try {
    // Fix Communication Underwater lesson content
    ...
    for (const track of allTracks) {
      ...
      // Fix Communication Underwater lesson
      if (track.slug === 'air-diver-certification') {
        const commLesson = trackLessons.find(l => l.title.includes('Communication'));
        if (commLesson) {
          await db.update(lessons)
            .set({ content: communicationLessonContent })
            .where(eq(lessons.id, commLesson.id));
          console.log(`   ✅ Fixed: ${commLesson.title}`);
        }
      }
    }

    console.log('\n✅ All alignment fixes completed!');
  } catch (error) {
