...
// Comprehensive lesson templates for each track
const lessonTemplates: { [key: string]: Array<{ title: string; content: string }> } = {
  'ndt-inspection': [
...
## AI Tutor: Alex Johnson - Life Support Systems Specialist
*Expert in diving life support operations, gas management, and environmental control systems*

Welcome to Lesson ${num} of Assistant Life Support Technician training! I'm Alex Johnson, your AI tutor. Today we'll cover ${topic} for life support operations.
...
*Emergency medicine specialist focused on diving-related medical emergencies*
...
*Expert in dive operations management, risk assessment, and safety protocols*
...
*Expert in air diving operations, gas management, and safety protocols*
...
*Expert in saturation diving operations, life support systems, and deep-sea operations*
...
async function create12LessonsPerTrack() {
  console.log('🌱 Creating 12 lessons for each track...\n');
  ...
  try {
    ...
    for (const track of allTracks) {
      ...
      const lessonsToAdd = lessonTemplates[track.slug] || [];
      
      if (lessonsToAdd.length === 0) {
        // Create 12 placeholder lessons for tracks without templates
        for (let i = 1; i <= 12; i++) {
          await db.insert(lessons).values({
            trackId: track.id,
            title: `${track.title} - Lesson ${i}`,
            order: i,
            content: `# ${track.title} - Lesson ${i}\n\nWelcome to Lesson ${i} of ${track.title}. This lesson is currently under development. Content will be added soon.`,
            estimatedMinutes: 60,
            isRequired: true,
          });
        }
        console.log(`   ✅ Added 12 placeholder lessons`);
        totalLessonsAdded += 12;
      } else {
        // Add lessons from template
        ...
        totalLessonsAdded += lessonsToAdd.length;
      }
      console.log('');
    }
...
create12LessonsPerTrack()
  .then(() => {
    console.log('\n✅ All tracks now have 12 lessons!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('\n💥 Failed:', error);
